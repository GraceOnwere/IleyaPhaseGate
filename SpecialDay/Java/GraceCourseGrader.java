...
public class GraceCourseGrader{
    ...
    public static double getBestQuiz(double [] averagePerQuiz){

    double maxmiumAveragePerQuiz = averagePerQuiz[0];

    for (int count = 1; count < averagePerQuiz.length; count++){

    if (averagePerQuiz[count] > maxmiumAveragePerQuiz){

        maxmiumAveragePerQuiz = averagePerQuiz[count];
        }
    }

    return maxmiumAveragePerQuiz;
